...
CustomExtenders.ListCompleteBehavior = function (element) {
    CustomExtenders.ListCompleteBehavior.initializeBase(this, [element]);

    this._headerCheckBoxID = null;
    this._rowCheckBoxID = null;
};
CustomExtenders.ListCompleteBehavior.prototype = {
    initialize: function () {
        var listBoxId = this.get_element().id;

        var controlWidth = this.get_ControlWidth();
        var jsonUrl = this.get_JsonUrl();
        var isCacheAllowed = this.get_IsCacheAllowed();
        var inputFieldTabIndex = this.get_InputFieldTabIndex();
        var tipForSelectAll = this.get_TipForSelectAll();
        var maxItemsAllowed = this.get_MaxItemsAllowed();
        var maxItemsShown = this.get_MaxItemsShown();
        var tipForItemSelection = this.get_TipForItemSelection();
        var filterSelectedItems = this.get_FilterSelectedItems();
        var isCaseSensitive = this.get_IsCaseSensitive();
        var isFirstItemSelected = this.get_IsFirstItemSelected();
        var allowAddingOnComma = this.get_AllowAddingOnComma();
        var allowAddingOnTab = this.get_AllowAddingOnTab();
        var allowAddingNewElements = this.get_AllowAddingNewElements();
        var maxDropdownHeight = this.get_MaxDropdownHeight();
        var minInputLength = this.get_MinInputLength();

        var filterForListCompleteControl = "#" + listBoxId + "+ul.holder";
        //ul is just a prefix for the "UnroderList" this plugin creates in place of actual control (TextBox or ListBox)

        //check if autocomplete control is already present(in case of partial page load)
        if ($(filterForListCompleteControl).length <= 0) {
            $(this.get_element()).fcbkcomplete({
                json_url: jsonUrl,
                width: controlWidth,
                cache: isCacheAllowed,
                height: maxDropdownHeight,
                newel: allowAddingNewElements,
                addontab: allowAddingOnTab,
                addoncomma: allowAddingOnComma,
                firstselected: isFirstItemSelected,
                filter_case: isCaseSensitive,
                filter_selected: filterSelectedItems,
                complete_text: tipForItemSelection,
                select_all_text: tipForSelectAll,
                maxshownitems: maxItemsShown,
                maxitems: maxItemsAllowed,
                input_tabindex: inputFieldTabIndex,
                input_min_size: parseFloat(minInputLength) - 1
                //                oncreate: null,
                //                onselect: function (item) {
                //                    if ($(hiddenControl)[0].value == "") {
                //                        $(hiddenControl)[0].value = ExtractItemValue(item);
                //                    } else {
                //                        $(hiddenControl)[0].value = $(hiddenControl)[0].value + "," + ExtractItemValue(item);
                //                    }
                //                },
                //                onremove: function (item) {
                //                    var textControl = hiddenControl;
                //                    item = ExtractItemValue(item);
                //                    var currentValueSet = $(textControl)[0].value;
                //                    var newValueSet = "";

                //                    if (currentValueSet != "") {
                //                        var currValueArray = currentValueSet.split(",");
                //                        if (currValueArray.length > 0) {
                //                            var i = 0;
                //                            for (i = 0; i < currValueArray.length; i++) {
                //                                if (currValueArray[i] != item) {
                //                                    if (currValueArray[i] != "") {
                //                                        if (i == 0) {
                //                                            newValueSet += currValueArray[i];
                //                                        } else {
                //                                            if (newValueSet == "") {
                //                                                newValueSet += currValueArray[i];
                //                                            } else {
                ...
                //                        }
                //                    }
                //                    $(textControl)[0].value = newValueSet;
                //                }
            });


            ////            // adding seleceted values as comma separated list in hidden text box
            ////            if ($("#" + $(mainControl).attr("id") + ">option[selected=selected]").length > 0) {
            ////                $($("#" + $(mainControl).attr("id") + ">option[selected=selected]")).each(function(index) {

            ////                    if ($(hiddenControl)[0].value == "") {
            ////                        $(hiddenControl)[0].value = $(this)[0].value;
            ////                    } else {
            ////                        $(hiddenControl)[0].value = $(hiddenControl)[0].value + "," + $(this)[0].value;
            ////                    }
            ////                });
            ////            }
    ...
    dispose: function () {
        // Cleanup code 
        CustomExtenders.ListCompleteBehavior.callBaseMethod(this, 'dispose');
    },

    // Property accessors 

    get_ControlWidth: function () { return this._controlWidth; },
    set_ControlWidth: function (value) { this._controlWidth = value; },

    get_JsonUrl: function () { return this._jsonUrl; },
    set_JsonUrl: function (value) { this._jsonUrl = value; },

    get_IsCacheAllowed: function () { return this._isCacheAllowed; },
    set_IsCacheAllowed: function (value) { this._isCacheAllowed = value; },

    get_InputFieldTabIndex: function () { return this._inputFieldTabIndex; },
    set_InputFieldTabIndex: function (value) { this._inputFieldTabIndex = value; },

    get_TipForSelectAll: function () { return this._tipForSelectAll; },
    set_TipForSelectAll: function (value) { this._tipForSelectAll = value; },

    get_MaxItemsAllowed: function () { return this._maxItemsAllowed; },
    set_MaxItemsAllowed: function (value) { this._maxItemsAllowed = value; },

    get_MaxItemsShown: function () { return this._maxItemsShown; },
    set_MaxItemsShown: function (value) { this._maxItemsShown = value; },

    get_TipForItemSelection: function () { return this._tipForItemSelection; },
    set_TipForItemSelection: function (value) { this._tipForItemSelection = value; },

    get_FilterSelectedItems: function () { return this._filterSelectedItems; },
    set_FilterSelectedItems: function (value) { this._filterSelectedItems = value; },

    get_IsCaseSensitive: function () { return this._isCaseSensitive; },
    set_IsCaseSensitive: function (value) { this._isCaseSensitive = value; },

    get_IsFirstItemSelected: function () { return this._isFirstItemSelected; },
    set_IsFirstItemSelected: function (value) { this._isFirstItemSelected = value; },

    get_AllowAddingOnComma: function () { return this._allowAddingOnComma; },
    set_AllowAddingOnComma: function (value) { this._allowAddingOnComma = value; },

    get_AllowAddingOnTab: function () { return this._allowAddingOnTab; },
    set_AllowAddingOnTab: function (value) { this._allowAddingOnTab = value; },

    get_AllowAddingNewElements: function () { return this._allowAddingNewElements; },
    set_AllowAddingNewElements: function (value) { this._allowAddingNewElements = value; },

    get_MaxDropdownHeight: function () { return this._maxDropdownHeight; },
    set_MaxDropdownHeight: function (value) { this._maxDropdownHeight = value; },

    get_MinInputLength: function () { return this._minInputLength; },
    set_MinInputLength: function (value) { this._minInputLength = value; }
};
CustomExtenders.ListCompleteBehavior.registerClass('CustomExtenders.ListCompleteBehavior', Sys.Extended.UI.BehaviorBase);
    ...
    $.fn.fcbkcomplete = function (opt) {
        return this.queue(function () {
            ...
            function addItem(title, value, preadded, locked, focusme) {
                ...
                ////                // added by danish160 1/12/11 START
                ...
                ////                    }
                ////                }
                ////                // added by danish160 1/12/11 END
